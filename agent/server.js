...
const { corpusMeta } = require("./retrieve");
  ...
  if (req.method === "POST" && req.url === "/chat") {
    ...
    req.on("end", async () => {
      try {
        ...
        // Rebuilt per turn: in grounded mode the excerpts depend on the question.
        const { prompt } = buildSystemPrompt(lastUserMessage);
...
server.listen(PORT, () => {
  ...
  if (meta) {
    console.log(
      `Knowledge: ${meta.documentCount} document(s) / ${meta.chunkCount} chunk(s) ` +
        `from ${meta.sourceDir}, ingested ${meta.generatedAt.slice(0, 10)}`
    );
  } else {
    console.log("Knowledge: no corpus ingested yet — falling back to profile.js. Run `npm run ingest`.");
  }
});
